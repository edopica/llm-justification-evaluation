#!/bin/bash
#SBATCH --job-name=essay_evaluation_answer_generation
#SBATCH --output=logs/essay_answer_%j.out
#SBATCH --error=logs/essay_answer_%j.err
...
while IFS= read -r model; do
    ...
    python scripts/essays_evaluation_answer_generator.py \
        --model "$model" \
        --range 0:4
