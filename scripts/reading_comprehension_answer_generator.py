...
def main():
    ...
    prompt_file_path = os.path.join(project_root, "data", "prompts", "reading_comprehension_prompt.txt")
    input_csv_path = os.path.join(project_root, "data", "datasets", "reading_comprehension_pool.csv")
    output_csv_path = os.path.join(project_root, "data", "generated_data", "reading_comprehension_answers.csv")

    output_header = ["QuestionID", "PassageID", "response", "model", "time_taken_seconds"]
            ...
            with open(input_csv_path, "r", newline='', encoding="utf-8") as infile:
                ...
                for i, row in enumerate(csv_reader):
                    ...
                    try:
                        ...
                        prompt_text = row['PassageText']
                        essay_text = row['QuestionText']
                        problem_text = f"Prompt: {prompt_text}\n\nEssay: {essay_text}"
                    except KeyError as e:
                        print(f"Skipping row {current_row_index + 1} in {input_csv_path} due to missing column: {e}. Expected 'QuestionID', 'prompt', 'question'.")
                        continue
